...
def main():
    ...
    gol.set_value('gol_test_file',data_args.test_file)
    ...
    def preprocess_function_train(examples):
        ...
        for i in range(len(examples[prompt_column])):
            if examples[prompt_column][i] and examples[response_column][i]:
                ...
                if 'classify' in training_args.output_dir or 'identify' in training_args.output_dir:
                    indices_ide, indices_cla = [],[]
                    flag = 'ide'
                    for index, value in enumerate(b_ids[1:]):
                        if value == 12:
                            flag = 'cla'
                            continue
                        elif value == 4:
                    ...
                    label_mask = []
                    if 'classify' in training_args.output_dir:
                        indices_mask = indices_ide
                    elif 'identify' in training_args.output_dir:
                        indices_mask = indices_cla
                    for index, value in enumerate(input_ids[mask_position+1:]):
                        if index - 2 in indices_mask:
                            label_mask.append(-100)
                        else:
                            label_mask.append(value)
                    labels = [-100] * context_length + label_mask
                else:
                    labels = [-100] * context_length + input_ids[mask_position+1:]

                d_len = max_seq_length - len(input_ids)
                input_ids = input_ids + [tokenizer.pad_token_id] * d_len
                labels = labels + [tokenizer.pad_token_id] * d_len
    ...
    if training_args.do_predict:
        ...
        print('Length: ',len(predict_dataset))
            ...
            if training_args.predict_with_generate:
                ...
                with open(output_prediction_file, "w", encoding="utf-8") as writer:
                    for idx, (p, l) in enumerate(zip(predictions, labels)):
                        ...
                        samp["target"] = p
                        test_predictions_list.append(samp)
                        res = json.dumps(samp, ensure_ascii=False)
                        writer.write(f"{res}\n")

    # return results
    return model_args, data_args, training_args, test_predictions_list
...
if __name__ == "__main__":
    ...
    debug_mode = 0
    success_flag = 1
    standard_path = data_args.test_file.replace("test.json","test_struc.json")
    # submit_path = training_args.output_dir + "test_cd_struc.json"
    out_path = training_args.output_dir + "test_cd_result.json"
    # 加载金标准文件和预测文件
